...
if [[ -z "${DB_ROOT_PASS:-}" ]]; then
  read -r -s -p "DB root password: " DB_ROOT_PASS
  echo
fi
...
CONNECT_OPTS=(-u "$DB_ROOT_USER" "-p${DB_ROOT_PASS}")
if [[ -S "$DB_SOCKET" ]]; then
...
if [[ "$APPLY_DEV" =~ ^[Yy]$ ]]; then
  ...
  "$DB_CMD" --force "${CONNECT_OPTS[@]}" "$EPMS_DEV_DB" < "$DDL_FILE"
  echo "==> Applying DML to ${EPMS_DEV_DB}..."
  "$DB_CMD" --force "${CONNECT_OPTS[@]}" "$EPMS_DEV_DB" < "$DML_FILE"
fi
...
if [[ "$APPLY_PROD" =~ ^[Yy]$ ]]; then
  ...
  "$DB_CMD" --force "${CONNECT_OPTS[@]}" "$EPMS_PROD_DB" < "$DDL_FILE"
  echo "==> Applying DML to ${EPMS_PROD_DB}..."
  "$DB_CMD" --force "${CONNECT_OPTS[@]}" "$EPMS_PROD_DB" < "$DML_FILE"
fi
