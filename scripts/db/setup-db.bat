...
set /p DB_ROOT_PASS=DB root password:
...
%DB_CMD% -h %DB_HOST% -P %DB_PORT% -u %DB_ROOT_USER% -p%DB_ROOT_PASS% ^
...
if /I "%APPLY_DEV%"=="Y" (
  ...
  %DB_CMD% -h %DB_HOST% -P %DB_PORT% -u %DB_ROOT_USER% -p%DB_ROOT_PASS% --force %EPMS_DEV_DB% < "%DDL_FILE%"
  echo ==> Applying DML to %EPMS_DEV_DB%...
  %DB_CMD% -h %DB_HOST% -P %DB_PORT% -u %DB_ROOT_USER% -p%DB_ROOT_PASS% --force %EPMS_DEV_DB% < "%DML_FILE%"
)
...
if /I "%APPLY_PROD%"=="Y" (
  ...
  %DB_CMD% -h %DB_HOST% -P %DB_PORT% -u %DB_ROOT_USER% -p%DB_ROOT_PASS% --force %EPMS_PROD_DB% < "%DDL_FILE%"
  echo ==> Applying DML to %EPMS_PROD_DB%...
  %DB_CMD% -h %DB_HOST% -P %DB_PORT% -u %DB_ROOT_USER% -p%DB_ROOT_PASS% --force %EPMS_PROD_DB% < "%DML_FILE%"
)
